# OSF Project Master Wiki Guide
## Sustainable Building Materials in Africa (2015–2025): A Systematic Review

**Project URL:** [https://osf.io/dvukp/](https://osf.io/dvukp/)  
**Lead Author:** Agbeniga Agboola ([ORCID: 0009-0005-6878-1661](https://orcid.org/0009-0005-6878-1661) | [emmanuelagbeniga@gmail.com](mailto:emmanuelagbeniga@gmail.com))  
**Affiliation:** Independent Researcher and Civil / Site Engineer  
**Permanent Repository DOI:** [10.5281/zenodo.22771539](https://doi.org/10.5281/zenodo.22771539)  
**License:** Creative Commons Attribution 4.0 International (CC-BY 4.0)  

---

> [!TIP]
> **How to Use This Guide:**  
> This file contains the ready-to-use Wiki text for the **Home Wiki** as well as each of the **8 child components** on your OSF project page.  
> You can also find individual component wiki files ready for one-click copy-pasting in the [`OSF_Wikis/`](file:///c:/Users/emman/OneDrive/Documents/SustainableMaterials_Africa_2015-2025/OSF_Wikis/) directory:
> - [`OSF_Wikis/00_Home_Wiki.md`](file:///c:/Users/emman/OneDrive/Documents/SustainableMaterials_Africa_2015-2025/OSF_Wikis/00_Home_Wiki.md) $\to$ Main Project Home Wiki (`https://osf.io/dvukp/wiki/home/`)
> - [`OSF_Wikis/01_Protocol_Wiki.md`](file:///c:/Users/emman/OneDrive/Documents/SustainableMaterials_Africa_2015-2025/OSF_Wikis/01_Protocol_Wiki.md) $\to$ `01_Protocol` Wiki tab
> - [`OSF_Wikis/02_SearchLogs_Wiki.md`](file:///c:/Users/emman/OneDrive/Documents/SustainableMaterials_Africa_2015-2025/OSF_Wikis/02_SearchLogs_Wiki.md) $\to$ `02_SearchLogs` Wiki tab
> - [`OSF_Wikis/03_Screening_Wiki.md`](file:///c:/Users/emman/OneDrive/Documents/SustainableMaterials_Africa_2015-2025/OSF_Wikis/03_Screening_Wiki.md) $\to$ `03_Screening` Wiki tab
> - [`OSF_Wikis/04_DataExtraction_Wiki.md`](file:///c:/Users/emman/OneDrive/Documents/SustainableMaterials_Africa_2015-2025/OSF_Wikis/04_DataExtraction_Wiki.md) $\to$ `04_DataExtraction` Wiki tab
> - [`OSF_Wikis/05_QualityAppraisal_Wiki.md`](file:///c:/Users/emman/OneDrive/Documents/SustainableMaterials_Africa_2015-2025/OSF_Wikis/05_QualityAppraisal_Wiki.md) $\to$ `05_QualityAppraisal` Wiki tab
> - [`OSF_Wikis/06_Analysis_Wiki.md`](file:///c:/Users/emman/OneDrive/Documents/SustainableMaterials_Africa_2015-2025/OSF_Wikis/06_Analysis_Wiki.md) $\to$ `06_Analysis` Wiki tab
> - [`OSF_Wikis/07_Manuscript_Wiki.md`](file:///c:/Users/emman/OneDrive/Documents/SustainableMaterials_Africa_2015-2025/OSF_Wikis/07_Manuscript_Wiki.md) $\to$ `07_Manuscript` Wiki tab
> - [`OSF_Wikis/08_Supplements_Wiki.md`](file:///c:/Users/emman/OneDrive/Documents/SustainableMaterials_Africa_2015-2025/OSF_Wikis/08_Supplements_Wiki.md) $\to$ `08_Supplements` Wiki tab

---

# Table of Contents
1. [Master Project Home Wiki](#1-master-project-home-wiki)
2. [Component 01: Protocol Wiki](#2-component-01-protocol-wiki)
3. [Component 02: SearchLogs Wiki](#3-component-02-searchlogs-wiki)
4. [Component 03: Screening Wiki](#4-component-03-screening-wiki)
5. [Component 04: DataExtraction Wiki](#5-component-04-dataextraction-wiki)
6. [Component 05: QualityAppraisal Wiki](#6-component-05-qualityappraisal-wiki)
7. [Component 06: Analysis Wiki](#7-component-06-analysis-wiki)
8. [Component 07: Manuscript Wiki](#8-component-07-manuscript-wiki)
9. [Component 08: Supplements Wiki](#9-component-08-supplements-wiki)

---

# 1. Master Project Home Wiki
*(Copy this block into `https://osf.io/dvukp/wiki/home/`)*

```markdown
# Sustainable Building Materials in Africa (2015–2025): A Systematic Review

**Lead Author:** Agbeniga Agboola ([ORCID: 0009-0005-6878-1661](https://orcid.org/0009-0005-6878-1661) | emmanuelagbeniga@gmail.com)  
**Affiliation:** Independent Researcher and Civil / Site Engineer  
**Registered OSF Project:** https://osf.io/dvukp/  
**Permanent Repository DOI:** 10.5281/zenodo.22771539 (https://doi.org/10.5281/zenodo.22771539)  
**Standards:** PRISMA 2020 Statement | Joanna Briggs Institute (JBI) Critical Appraisal  
**License:** Creative Commons Attribution 4.0 International (CC-BY 4.0)  

---

## Executive Overview
Sub-Saharan Africa is experiencing the fastest urban population growth on Earth, requiring an estimated 250 million new housing units by 2050. However, continental construction remains heavily dependent on Ordinary Portland Cement (OPC) concrete and sandcrete masonry blocks, driving severe environmental degradation through illegal river sand dredging, draining foreign exchange reserves via clinker importation, and precipitating frequent building collapses due to substandard artisanal block casting (yielding 1.0–1.8 MPa vs. 2.5 MPa NIS 87 standard).

This repository contains the complete computational workflow, bibliographic logs, screening decisions, extraction registers, quality appraisals, synthesis tables, high-resolution figures, and manuscript deliverables for the decadal systematic review of sustainable building materials in Africa (2015–2025).

Synthesizing **9,112 raw bibliographic records** across Google Scholar, OpenAlex, DOAJ, and AJOL, the review extracts and appraises **2,822 empirical laboratory and field studies** across 29 standardized parameters.

---

## Component Structure & Navigation
1. **01_Protocol:** Formally registered systematic review protocol (Protocol_SR_SBM_Africa_v1_2025-09-24.pdf; 872.2 kB) establishing PICO/CCo criteria.
2. **02_SearchLogs:** Multi-database search queries and algorithmic deduplication audit (9,112 raw citations down to 7,283 unique records; 1,829 duplicates eliminated).
3. **03_Screening:** Two-stage title, abstract, and full-text screening registers (4,461 excluded with standardized reason codes: EX-GEO, EX-MAT, EX-THEORY, EX-DUP).
4. **04_DataExtraction:** Master 29-field relational database (Extraction_Completed_Master.xlsx) spanning 2,822 empirical studies across 5 material classes.
5. **05_QualityAppraisal:** Joanna Briggs Institute (JBI) 9-item critical appraisal checklist scoring logs for quasi-experimental trials.
6. **06_Analysis:** 6 comprehensive synthesis worksheets (Synthesis_Tables.xlsx) and 5 publication-ready 300-DPI figures.
7. **07_Manuscript:** Complete APA 7th Edition manuscript (Manuscript_Draft_v1.docx; 10,448 words, 87 references, 5 embedded tables) ready for peer review.
8. **08_Supplements:** Item-by-item 27-item PRISMA 2020 verification checklist and high-resolution flow diagram.

---

## Core Empirical Findings
* **Compressed Stabilized Earth Blocks (CSEBs):** 5–8% cement stabilization yields 3.5–9.2 MPa compressive strength and 7.0–11.5% water absorption, providing 2 to 3 times the strength of commercial sandcrete blocks (1.0–1.8 MPa) while utilizing 50–70% less cement.
* **Low-Carbon Binders (LC3 & Agro-Waste Pozzolans):** Limestone calcined clay cement concrete achieves 28–52 MPa, mitigating embodied carbon by 30–42%. Agricultural pozzolans (rice husk ash, sugarcane bagasse ash, palm oil fuel ash) achieve 22–44 MPa at 15–25% cement replacement.
* **Structural Bamboo:** Axial tensile capacities reach 120–240 MPa. When preserved with borax-boric acid, bamboo culms deliver net negative embodied carbon (-120 to -350 kg CO2-eq/m3).
* **Systemic Adoption Barriers:** Commercialization is hindered by outdated colonial prescriptive building codes (78%), fragmented agro-waste supply chains (72%), testing infrastructure deficits (64%), and social perception stigmas (61%).

---

## How to Cite
Agboola, A. (2026). *Sustainable Building Materials in Africa (2015–2025): A Systematic Review*. Open Science Framework. https://doi.org/10.5281/zenodo.22771539 (Project repository: https://osf.io/dvukp/).
```

---

# 2. Component 01: Protocol Wiki
*(Copy this block into `01_Protocol` Wiki tab)*

```markdown
# 01_Protocol: Systematic Review Protocol & Registration

**Lead Author:** Agbeniga Agboola (ORCID: 0009-0005-6878-1661)  
**Registered Protocol File:** Protocol_SR_SBM_Africa_v1_2025-09-24.pdf (872.2 kB)  
**Official Registration Timestamp:** September 24, 2025  

## Scope & Methodological Governance
This component houses the foundational systematic review protocol pre-registered in accordance with PRISMA-P and PICO/CCo standards:
* **Context:** Sovereign territories of Africa across five geographic regions (North, West, Central, East, Southern) and diverse climatic zones.
* **Intervention:** Empirical investigations on 5 sustainable material classes: (1) Earth-Based Masonry, (2) Low-Carbon Binders/LC3, (3) Recycled & Industrial Wastes, (4) Renewable Bio-Based Systems, and (5) Smart/Innovative Materials.
* **Comparator:** Conventional carbon-intensive African baselines: Ordinary Portland Cement (OPC) concrete (BS EN 206 C20/25) and commercial sandcrete blocks (NIS 87 / GS 297).
* **Outcomes:** 28-day compressive/flexural strength, density, water absorption, thermal conductivity, embodied carbon (kg CO2-eq/m3), GWP % reduction, and systemic adoption barriers.

## Open Science Integrity
The primary protocol PDF is permanently locked to preserve the September 24, 2025 timestamp, guaranteeing that study eligibility criteria and search query architectures were defined prior to data synthesis. Dual-redundantly archived on Zenodo under DOI: 10.5281/zenodo.22771539.
```

---

# 3. Component 02: SearchLogs Wiki
*(Copy this block into `02_SearchLogs` Wiki tab)*

```markdown
# 02_SearchLogs: Multi-Database Search Audit & Deduplication

**Lead Author:** Agbeniga Agboola (ORCID: 0009-0005-6878-1661)  
**Raw Citations Identified:** 9,112  
**Duplicate Records Eliminated:** 1,829  
**Net Unique Records Advancing to Screening:** 7,283  

## Information Sources & Harvested Records
* **Google Scholar (5,747 citations):** Captured grey literature, African institutional repositories, and university theses using 5 thematic queries. Exported to `SearchLog_Master.csv`.
* **OpenAlex Scholarly API (2,802 citations):** Structured queries combining materials taxonomy and African sovereign country ISO codes across 250M+ works. Exported to `OpenAlex_Results_2026-09-15.csv`.
* **Directory of Open Access Journals [DOAJ] (419 citations):** High-standard open-access engineering publications. Exported to `DOAJ_Results_2026-09-15.csv`.
* **African Journals Online [AJOL] (144 citations):** Curated corpus of peer-reviewed publications across African national and university journals. Exported to `AJOL_Results_2026-09-15.csv`.

## Deduplication Pipeline (`Deduplication_Summary_2026-09-15.xlsx`)
1. Exact matching on standardized DOIs, normalized title strings, and author-year pairs.
2. Fuzzy Levenshtein string matching (threshold >= 0.90) for records lacking DOIs.
3. Total duplicates removed: 1,829 (Google Scholar: 1,180; OpenAlex: 542; DOAJ: 87; AJOL: 20).
```

---

# 4. Component 03: Screening Wiki
*(Copy this block into `03_Screening` Wiki tab)*

```markdown
# 03_Screening: Two-Stage Screening Registers & Audit Logs

**Lead Author:** Agbeniga Agboola (ORCID: 0009-0005-6878-1661)  
**Unique Records Screened:** 7,283  
**Excluded Records:** 4,461  
**Eligible Full-Text Empirical Studies:** 2,822  

## Screening Protocol
* **Stage 1 (Title/Abstract Screening):** All 7,283 unique records screened against inclusion criteria. 4,461 records were excluded with standardized reason codes:
  - `EX-GEO` (Non-African Geographic Context): n = 1,842
  - `EX-MAT` (Out-of-Scope Non-Construction Topics): n = 1,328
  - `EX-THEORY` (Purely Speculative / Non-Empirical): n = 894
  - `EX-DUP` (Duplicate / Preliminary Notes): n = 397
* **Stage 2 (Full-Text Retrieval & Eligibility):** All 2,822 retrieved empirical studies satisfied experimental completeness criteria and advanced to data extraction.

## Included Registers
* `TitleAbstract_ScreeningLog.xlsx`: Complete 7,283-record screening audit with decisions and reason codes.
* `FullText_ScreeningLog.xlsx`: 2,822 confirmed candidate studies.
* `Screening_Audit_Summary_2026-09-15.xlsx`: Tabulated breakdown of screening metrics.
```

---

# 5. Component 04: DataExtraction Wiki
*(Copy this block into `04_DataExtraction` Wiki tab)*

```markdown
# 04_DataExtraction: 29-Field Extraction Master Dataset

**Lead Author:** Agbeniga Agboola (ORCID: 0009-0005-6878-1661)  
**Master Dataset:** `Extraction_Completed_Master.xlsx`  
**Dataset Scale:** 2,822 studies x 29 relational variables  

## The 29 Standardized Extraction Fields
1. Bibliographic: ID, Author, Year, Title, Journal/Source, DOI
2. Context: Country, Region, Climate Context (Köppen-Geiger)
3. Taxonomy: Material Class, Material Specification, Study Aim, Study Design, Methods Summary, Sample Size, Duration, Scale
4. Performance Metrics: Technical Metrics (Compressive, Flexural, Density, Absorption, Thermal), Environmental Metrics (Embodied Carbon, Energy, LCA boundary), Economic Metrics, Social/Practical Notes, Comparator Baseline, Standards Referenced
5. Synthesis & Quality: Key Findings, Limitations, Funding Declared, Data Availability, Ethics Approval, Quality Tool Used

## Proportional Representation across 2,822 Studies
* Low-Carbon Binders & SCMs: 35.3% (n = 995)
* Earth-Based Masonry: 31.3% (n = 884)
* Recycled & Waste Composites: 15.7% (n = 444)
* Renewable Bio-Based Systems: 12.7% (n = 358)
* Smart & Innovative Systems: 5.0% (n = 141)
```

---

# 6. Component 05: QualityAppraisal Wiki
*(Copy this block into `05_QualityAppraisal` Wiki tab)*

```markdown
# 05_QualityAppraisal: Joanna Briggs Institute (JBI) Risk of Bias

**Lead Author:** Agbeniga Agboola (ORCID: 0009-0005-6878-1661)  
**Appraisal File:** `Appraisal_Summary.xlsx`  
**Tool:** JBI Critical Appraisal Checklist for Quasi-Experimental Studies (9 Criteria)  
**Evaluated Studies:** 2,822 empirical publications  

## JBI 9-Criteria Summary
* Q1 (Causality): 100.0% satisfied
* Q2 (Baseline Material Similarity): 98.2% satisfied
* Q3 (Comparison Consistency): 98.9% satisfied
* Q4 (Control Group Included): 99.8% satisfied (OPC concrete or unstabilized earth)
* Q5 (Multiple Post-Intervention Curing Points): 97.1% satisfied
* Q6 (Complete Follow-Up Across Specimens): 99.4% satisfied
* Q7 (Standardized Measurement Protocols): 100.0% satisfied
* Q8 (Reliable Testing Instrumentation - ASTM/BS): 99.8% satisfied
* Q9 (Statistical Reporting of Variance): 77.5% satisfied

## Synthesis Insight
100% of included studies exhibited Low Risk of Bias on core experimental parameters. The primary continental reporting deficiency is statistical reporting of variance (Q9; 77.5%), where single mean values are frequently reported without standard deviations or ANOVA significance testing.
```

---

# 7. Component 06: Analysis Wiki
*(Copy this block into `06_Analysis` Wiki tab)*

```markdown
# 06_Analysis: Synthesis Tables & Publication-Grade Figures

**Lead Author:** Agbeniga Agboola (ORCID: 0009-0005-6878-1661)  
**Master Synthesis Tables:** `Synthesis_Tables.xlsx` (6 comprehensive worksheets)  
**Figures:** 5 publication figures rendered at 300 DPI in `06_Analysis/Figures/`  

## Synthesis Worksheets (`Synthesis_Tables.xlsx`)
1. Geographic_Distribution: Country and regional outputs (West Africa 38.2%, East 21.4%, Southern 18.6%, North 14.1%, Central 7.7%).
2. Taxonomy_Breakdown: Proportions, feedstocks, and processing mechanisms.
3. Mechanical_Benchmarks: 28-day compressive strength, flexural strength, density, absorption, and thermal conductivity.
4. LCA_Environmental_Profiles: Embodied carbon (kg CO2-eq/m3), energy (MJ/m3), GWP reductions, and carbon sequestration.
5. Barriers_Enablers_Matrix: Regulatory (78%), Supply Chain (72%), Technical (64%), and Socio-Cultural (61%) barriers vs. enablers.
6. JBI_Quality_Summary: Methodological quality scores across classes.

## High-Resolution Academic Figures (300 DPI)
* `Fig1_PRISMA_2020_Flow_Diagram.png`: Full PRISMA flow chart (9,112 -> 1,829 removed -> 7,283 screened -> 4,461 excluded -> 2,822 included).
* `Fig2_Geographic_Distribution.png`: Spatial concentration across African regions and national hubs.
* `Fig3_Material_Class_Taxonomy_Distribution.png`: Proportions of the 5 material classes.
* `Fig4_Compressive_Strength_Benchmark_Boxplot.png`: Boxplot distribution demonstrating CSEB superiority (3.5–9.2 MPa) over commercial sandcrete blocks (1.0–1.8 MPa).
* `Fig5_Barriers_Enablers_Taxonomy_Radar.png`: Multi-axis radar diagram of systemic adoption barriers.
```

---

# 8. Component 07: Manuscript Wiki
*(Copy this block into `07_Manuscript` Wiki tab)*

```markdown
# 07_Manuscript: Comprehensive Systematic Review Deliverables

**Lead Author:** Agbeniga Agboola (ORCID: 0009-0005-6878-1661)  
**Affiliation:** Independent Researcher and Civil / Site Engineer  
**Primary Deliverable:** `Manuscript_Draft_v1.docx` (Canonical Word Document)  
**Markdown Version:** `Manuscript_Draft_v1.md`  
**Scale:** 10,448 words | 318 paragraphs | 87 peer-reviewed references | 5 embedded synthesis tables  
**Formatting Standard:** Strict APA 7th Edition (Double-spaced, running head, 1-inch margins, APA 3-line table borders, hanging indents)  

## Manuscript Structure
* Title Page & Structured Abstract (300 words)
* Section 1: Introduction & Continental Imperative (Urban explosion, clinker forex drain, sand dredging, artisanal sandcrete collapses, PICO/CCo framework)
* Section 2: PRISMA 2020 Systematic Methodology (Search architecture, deduplication, 2-stage screening, 29-field extraction, JBI appraisal)
* Section 3: Empirical Results & Systematic Synthesis (Geographic distribution, taxonomy, earth masonry benchmarks, LC3/SCMs kinetics, recycled composites, structural bamboo, PCMs, LCA carbon profiles; Tables 1–4)
* Section 4: Critical Discussion, Risk of Bias & The Sandcrete Paradox (JBI scores, Table 5, market failure of sandcrete, 4-pillar barriers matrix, review limitations)
* Section 5: Conclusions & Strategic Action Roadmap (2026–2030 priorities: ARSO standards gazetting, 25% green public procurement quotas, PPP calcination hubs, TVET artisan curricula)
* Declarations & 87-Reference Bibliography (APA 7th Edition with active DOIs)

## Archival
Permanently preserved on Zenodo under persistent DOI: 10.5281/zenodo.22771539.
```

---

# 9. Component 08: Supplements Wiki
*(Copy this block into `08_Supplements` Wiki tab)*

```markdown
# 08_Supplements: Supplementary Verification Documentation

**Lead Author:** Agbeniga Agboola (ORCID: 0009-0005-6878-1661)  
**Standards:** PRISMA 2020 Statement (Page et al., 2021)  
**Key Files:** `PRISMA_2020_Checklist.docx` (and `.md`), `PRISMA_2020_Flow_Diagram_Completed.docx`  

## Supplementary Verification Overview
* **`PRISMA_2020_Checklist.docx`:** Item-by-item verification table confirming fulfillment of all 27 essential items and 42 sub-items of the PRISMA 2020 statement, mapped to exact section titles and tables in `Manuscript_Draft_v1.docx`.
* **`PRISMA_2020_Flow_Diagram_Completed.docx`:** High-resolution embedded supplementary flow chart documenting the full selection flow from 9,112 raw citations down to 2,822 included empirical studies.
```
